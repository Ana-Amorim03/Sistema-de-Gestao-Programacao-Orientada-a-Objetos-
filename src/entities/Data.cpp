...
bool Data::operator==(const Data &d2) const
{
	return (((((ano == d2.ano)&&(mes == d2.mes)&&(dia == d2.dia)&&(hora == d2.hora)&&
           (minuto == d2.minuto)&&(segundo == d2.segundo))||(ticks == d2.ticks)) && fracao_segundo==d2.fracao_segundo)) ;
}
...
bool Data::operator<(const Data &d2) const
{
	return ((ano<d2.ano)||(ano==d2.ano && mes<d2.mes)||(ano==d2.ano && mes==d2.mes && dia<d2.dia)||
           ...
           hora==d2.hora && minuto==d2.minuto && segundo<d2.segundo)||(ano==d2.ano && mes==d2.mes && dia==d2.dia &&
           hora==d2.hora && minuto==d2.minuto && segundo==d2.segundo && fracao_segundo < d2.fracao_segundo));
}
...
Data Data::operator-(const Data &d2) const
{
...
int Data::getHora() const { return hora; }

int Data::getMin() const{ return minuto; }

int Data::getSeg() const { return segundo; }

long Data::getTicks() const { return ticks; }

string Data::getFormato() const { return formatoData.getNome(); }

int Data::getFracaoSegundo() { return fracao_segundo; }
...
void Data::setFracaoSegundo(int fracao)
{
    fracao_segundo = fracao;
}

// Outros metodos:

bool Data::validaData() const
{
    int condicao= 1;
    ...
    if (condicao==0) { 
        char message[100] = "Data invalida.";
        throw ExecaoCustomizada(message);
    }
    return (condicao==1);
}
...
Data Data::dateNow()
{
    ...
    Data aux(tempo);
    if (aux.getHora()-3 < 0) {
        aux.setHora(aux.getHora()-3+24); // Converte a hora para UTC-03:00 (Horário de Brasília)
        aux.setDia(aux.getDia()-1); // Converte a hora para UTC-03:00 (Horário de Brasília)
    } else {
        aux.setHora(aux.getHora()-3); // Converte a hora para UTC-03:00 (Horário de Brasília)
    }
    aux.setFracaoSegundo(clock() % CLOCKS_PER_SEC);
    return aux;
}
...
string Data::getData() const{
   ...
   if (strcmp(formatoData.getNome().c_str(),"PT-BR")==0)
    {
      data = std::to_string(dia) + formatoData.getSeparadorData() + std::to_string(mes) + formatoData.getSeparadorData() + to_string(ano) + " " + to_string(hora)  + formatoData.getSeparadorHorario() + to_string(minuto) + formatoData.getSeparadorHorario() + to_string(segundo) + formatoData.getSeparadorHorario() + to_string(fracao_segundo);
...
int Data::diffData(Data d2) const {
